 ...
 * {@see RelayFrameType::HTTP_REQUEST} frame.
 *
 * Each HTTP_REQUEST frame payload is `[1-byte tag][data]`:
 *   - tag {@see self::TAG_HEAD} (0x01): `data` is {@see RelayHttpRequestHead} JSON.
 *   - tag {@see self::TAG_BODY} (0x02): `data` is raw body bytes (<= {@see self::MAX_BODY_CHUNK}).
 *   - tag {@see self::TAG_END}  (0x03): `data` is empty; the request is complete.
 *
 ...
 * @since 0.17.0
 */
final class RelayHttpRequestCodec
{
    /** HEAD chunk tag byte. */
    ...
    /**
     * Encode the HEAD chunk for an HTTP_REQUEST frame payload.
     *
     * @param RelayHttpRequestHead $head Request head (method + path + headers).
     *
     ...
     * @since 0.17.0
     */
    public static function encodeHead(RelayHttpRequestHead $head): string
    {
        return chr(self::TAG_HEAD) . $head->toJson();
    }

    /**
     * Encode a BODY chunk for an HTTP_REQUEST frame payload.
     *
    ...
    public static function encodeBody(string $bodyChunk): string
    {
        if (strlen($bodyChunk) > self::MAX_BODY_CHUNK) {
            throw new InvalidArgumentException(
                sprintf(
                    'RelayHttpRequestCodec: body chunk %d exceeds max %d bytes.',
                    strlen($bodyChunk),
                    self::MAX_BODY_CHUNK,
                ),
            );
    ...
    /**
     * Encode the END chunk for an HTTP_REQUEST frame payload.
     *
     ...
     * zero-length body).
     *
     * @param string $body Full request body bytes.
     *
    ...
    /**
     * Decode one HTTP_REQUEST frame payload into a typed chunk.
     *
     * @param string $payload The frame payload (tag + data).
     *
     * @return RelayHttpRequestChunk
     *
     * @throws InvalidArgumentException When the payload is empty or the tag is unknown.
     *
     * @since 0.17.0
     */
    public static function decode(string $payload): RelayHttpRequestChunk
    {
        if ($payload === '') {
            throw new InvalidArgumentException('RelayHttpRequestCodec: empty chunk payload.');
        }
        ...
        return match ($tag) {
            self::TAG_HEAD => new RelayHttpRequestChunk(
                RelayHttpRequestChunk::KIND_HEAD,
                RelayHttpRequestHead::fromJson($data),
                '',
            ),
            self::TAG_BODY => new RelayHttpRequestChunk(
                RelayHttpRequestChunk::KIND_BODY,
                null,
                $data,
            ),
            self::TAG_END => new RelayHttpRequestChunk(
                RelayHttpRequestChunk::KIND_END,
                null,
                '',
            ),
            default => throw new InvalidArgumentException(
                sprintf('RelayHttpRequestCodec: unknown chunk tag 0x%02X.', $tag),
            ),
        };
    }
}
